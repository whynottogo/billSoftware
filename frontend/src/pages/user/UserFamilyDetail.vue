  ...
  <div class="finance-page user-family-detail-page">
    <section v-if="family" class="finance-toolbar">
      ...
      <div class="finance-toolbar__actions">
        ...
        <button class="finance-button finance-button--primary" @click="copyInviteLink">复制邀请链接</button>
      </div>
    </section>
    ...
    <template v-if="family">
      ...
      <section class="page-card user-family-detail-page__member-panel">
        ...
        <div class="user-family-detail-page__member-list">
          <article v-for="member in family.members" :key="member.name" class="member-row">
    ...
    <section v-else class="page-card user-family-detail-page__missing">
      <h2>未找到该家庭</h2>
...
import { formatCurrency, getFamilyDetail, getFamilyMemberShare } from "@/utils/userFamilyMock";
...
export default {
  ...
  computed: {
    ...
    shareData() {
      ...
      return getFamilyMemberShare(
        this.familyId,
        this.shareParams.periodType,
        this.shareParams.periodKey,
        this.shareParams.metricType
      );
    }
  },
  watch: {
  ...
  methods: {
    ...
    loadFamily() {
      var detail = getFamilyDetail(this.familyId);
      ...
      this.selectedMonth = detail.monthOptions.length ? detail.monthOptions[0].key : "";
      this.selectedYear = detail.yearOptions.length ? detail.yearOptions[0].key : "";
      this.activeShareKey = "month-income";
    },
    setShareMode(mode) {
